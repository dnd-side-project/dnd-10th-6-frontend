...
import { useWikiContext, WikiProvider } from '@/contexts/wiki-provider'
...
import { AUTH, fadeInOutWithScaleProps } from '@/constants'
...
import FlowerLogo from '@/assets/logo/flower_logo.png'
...
function WikiLogo() {
  const { wikiType: contextWikiType } = useWikiContext()
  const wikiType = useMemo(() => contextWikiType ?? 'NAMUI', [contextWikiType])

  const Wiki = useCallback(() => {
    switch (wikiType) {
      case 'NAMUI':
        return (
          <motion.svg
            key="NAMUI"
            {...fadeInOutWithScaleProps}
            width="59"
            height="78"
            viewBox="0 0 59 78"
            fill="none"
  ...
  return (
    <div className="relative mt-16 flex aspect-square w-24 items-center justify-center rounded-2xl shadow-onboard">
